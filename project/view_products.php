...
if (isset($_POST['add_to_cart'])) {
   ...
   $name = filter_var($_POST['name'], FILTER_SANITIZE_STRING); // Capture name

   // Verify if the product is already in the cart
   $verify_cart = $conn->prepare("SELECT * FROM `cart` WHERE user_id = ? AND product_id = ?");   
   $verify_cart->execute([$user_id, $product_id]);

   // Check for maximum cart items
   ...
   if ($verify_cart->rowCount() > 0) {
       ...
       // Insert into cart
       $insert_cart = $conn->prepare("INSERT INTO `cart`(id, user_id, product_id, price, qty, name) VALUES(?,?,?,?,?,?)");
       $insert_cart->execute([$id, $user_id, $product_id, $fetch_price['price'], $qty, $name]);
